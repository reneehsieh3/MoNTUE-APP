    ...
    if (!permission.granted) {
        return (
            <View style={ styles.permission_container }>
                <Text style={{ fontSize: 16, color: colors.text }}>抱歉，我們需要相機權限才能掃描 QR code!</Text>
                <View style={styles.permission_buttom_container}>
                    <Pressable 
                        style={[styles.cancel_button, {backgroundColor: '#f8e364'}]}
                        onPress={requestPermission} 
                    >
                        <Text style={{ fontSize: 20, fontWeight: '600', color: colors.text }}>同意</Text>
                    </Pressable>
                    <Pressable 
                            style={styles.cancel_button}
                            onPress={onCancel} 
                        >
                        <Text style={{ fontSize: 20, fontWeight: '600', color: '#ff383c' }}>取消</Text>
                    </Pressable>
                </View>
            </View>
        );
        ...
        <View style={styles.camera_container}>
            ...
            <View style={styles.camera_shade_container}>
                <View style={[styles.camera_shade, { justifyContent: 'flex-end' }]}>
                    <Text style={{ fontSize: 15, color: colors.text, paddingBottom: 10 }}>請掃描作品說明板上的 QR code</Text>
                </View>
